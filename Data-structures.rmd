...
You should always get and get these attributes with their accessor functions: use `names(x)`, `class(x)` and `dim(x)`, not `attr(x, "names")`, `attr(x, "class")`, and `attr(x, "dim")`.
...
The factor is a vector that can contain only predefined values, and is R's structure for dealing with qualitative data. A factor is not a basic vector, but is build on top of an integer vector using the S3 class system, as described in the [OO-field-guide.html#s3]. Factors have two key attributes: their `class()`, "factor", which controls their behaviour; and their `levels()`, the set of allowed values.
